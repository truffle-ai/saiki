...
/**
 * System commands array - individual command definitions
 */
export const systemCommands: CommandDefinition[] = [
    {
        ...
        handler: async (args: string[], _agent: SaikiAgent) => {
            if (args.length === 0) {
                // This will need to be updated when we integrate back into the main file
                // For now, we'll pass an empty array since we can't import CLI_COMMANDS here
                console.log(
                    chalk.yellow('📋 Help system requires integration with main commands list')
                );
                console.log(
                    chalk.dim(
                        'Use specific command help like /session help, /model help, or /mcp help'
                    )
                );
                return true;
            }
            ...
            console.log(chalk.yellow(`📋 Command help for "${commandName}" not yet integrated`));
            console.log(chalk.dim('This will be available after system integration'));
            return true;
        },
    },
    {
