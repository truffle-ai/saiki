...
/**
 * Renders a user input area for composing and sending text messages with optional image attachments.
 *
 * Invokes the provided {@link onSend} callback with the trimmed text and optional image data when the user submits the input. Supports sending via Enter key (without Shift) or the send button. Allows users to attach an image, preview it, and remove it before sending. The textarea dynamically adjusts its height based on content.
 *
 * @param onSend - Callback invoked with the message content and optional image data when a message is sent.
 *
 * @remark
 * Only image files are accepted for attachment. If a non-image file is selected or if image processing fails, the error is logged and the image is not attached.
 */
