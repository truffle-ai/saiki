...
export const LLMConfigSchema = LLMConfigBaseSchema.superRefine((data, ctx) => {
    ...
    if (baseURLIsSet) {
        if (!supportsBaseURL(data.provider)) {
            ctx.addIssue({
                ...
                params: { code: SaikiErrorCode.INVALID_BASE_URL },
            });
        }
        ...
        ctx.addIssue({
            ...
            params: { code: SaikiErrorCode.MISSING_BASE_URL },
        });
    } else {
        if (!acceptsAnyModel(data.provider)) {
            ...
            if (!isValidProviderModel(data.provider, data.model)) {
                ctx.addIssue({
                    ...
                    params: { code: SaikiErrorCode.INCOMPATIBLE_MODEL_PROVIDER },
                });
            }
        }
            ...
            try {
                ...
                if (data.maxInputTokens! > cap) {
                    ctx.addIssue({
                        ...
                        params: { code: SaikiErrorCode.MAX_INPUT_TOKENS_EXCEEDED },
                    });
                }
            } catch (error: unknown) {
                // TODO: improve this
    ...
    if (!isRouterSupportedForProvider(data.provider, data.router)) {
        ...
        ctx.addIssue({
            ...
            params: { code: SaikiErrorCode.UNSUPPORTED_ROUTER },
        });
    }
})
