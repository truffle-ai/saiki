...
describe('Config Schemas', () => {
  it('contributorConfigSchema accepts valid config', () => {
    const valid = { id: 'user1', type: 'static', priority: 10, enabled: true, content: 'hello' };
  ...
  it('llmConfigSchema accepts valid string prompt', () => {
    const good = { provider: 'openai', model: 'o4-mini', systemPrompt: 'Hi', apiKey: 'key', maxIterations: 2 };
    expect(() => llmConfigSchema.parse(good)).not.toThrow();
  });

  it('llmConfigSchema accepts valid object prompt', () => {
    const objPrompt = { contributors: [{ id: 'c1', type: 'static', priority: 1, content: 'x' }] };
